...
def block_shape(ms: MatchSet) -> dict:
    ...
    rows.sort(
        key=lambda r: DEF_PHASE_ORDER.index(r["phase"]) if r["phase"] in DEF_PHASE_ORDER else 99
    )
    ...
    return {
        ...
        "clips_low": phase_episodes(
            ...
            lambda r: f"Низкий блок, ширина {r['width_out']:.0f} м",
            30,
        ),
        "clips": phase_episodes(
            out.nlargest(30, "length_out"),
            lambda r: f"{r['def_phase']}, блок {r['width_out']:.0f}×{r['length_out']:.0f} м",
            30,
        ),
    }
...
@metric(
    "defensive_line",
    "Высота последней линии обороны",
    "Оборона",
...
def defensive_line(ms: MatchSet) -> dict:
    ...
    return {
        ...
        "clips": episodes(
            ...
            lambda r: (
                f"Линия на {r['last_defensive_line_height_start']:.0f} м — {r.get('def_phase', '')}"
            ),
            30,
        ),
        "clips_late": episodes(
            ...
            lambda r: (
                f"{int(r['minute']) + 1}′, линия {r['last_defensive_line_height_start']:.0f} м"
            ),
            30,
        ),
    }
...
def formations(ms: MatchSet) -> dict:
    ...
    if d.empty:
        return {}
    s = d["defensive_structure"].astype(int).astype(str).map(lambda v: "-".join(list(v)))
    rows = share(s)[:6]
    ...
    return {
        ...
        "clips": episodes(
            ...
            lambda r: f"Оборона {rows[0]['key']} — {r.get('def_phase', '')}",
            30,
        ),
    }
