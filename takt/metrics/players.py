...
def player_table(ms: MatchSet) -> dict:
    ...
    for pid, mins in minutes.items():
        ...
        regains = (
            int(e["end_type"].isin(["direct_regain", "indirect_regain"]).sum())
            if "end_type" in e
            else 0
        )
...
def key_players(ms: MatchSet) -> dict:
    ...
    def top(df: pd.DataFrame, col: str, agg: str = "sum", n: int = 3) -> list[dict]:
    ...
    threat = top(own, "xt")
    breakers = (
        own.dropna(subset=["furthest_line_break"])
        if "furthest_line_break" in own
        else own.iloc[0:0]
    )
    behind = runs[runs["subtype"] == "behind"]

    def counts(df: pd.DataFrame, n: int = 3) -> list[dict]:
        d = df[df["player_id"].isin(ok)]
        if d.empty:
            return []
    ...
    if threat:
        best = threat[0]
        clips = episodes(
            own[own["player_id"] == best["id"]].sort_values("xt", ascending=False),
            lambda r: (
                f"{r['player_name']} — {r.get('phase', '')}, xT {float(r.get('xt') or 0):.3f}"
            ),
            20,
        )
        profiles.append({"role": "Главный источник угрозы", **best, "clips": clips})
    b = counts(breakers)
    if b:
        best = b[0]
        clips = episodes(
            breakers[breakers["player_id"] == best["id"]].sort_values("t"),
            lambda r: f"Взлом линии — {r['player_name']}, {r.get('phase', '')}",
            20,
        )
        profiles.append({"role": "Продвигает мяч через линии", **best, "clips": clips})
    r3 = counts(behind)
    if r3:
        best = r3[0]
        clips = episodes(
            behind[behind["player_id"] == best["id"]].sort_values("t"),
            lambda r: f"Забегание за спину — {r['player_name']}",
            20,
        )
        profiles.append({"role": "Бежит за спину чаще всех", **best, "clips": clips})
