...
@metric("pressing_profile", "Профиль прессинга", "Оборона",
...
def pressing_profile(ms: MatchSet) -> dict:
    ...
    return {
        ...
        if "danger_faced" in eng and eng["danger_faced"].notna().any() else None,
        ...
        if "goal_side_start" in eng and eng["goal_side_start"].notna().any() else None,
        "clips": episodes(
            eng[eng["end_type"].notna()].sort_values("t"),
            lambda r: f"{PRESS_RU.get(r['subtype'], r['subtype'])} — {r['player_name']}, "
                      f"{THIRD_RU.get(r.get('third_start'), '')}", 30),
    }


@metric("pressing_chains", "Цепочки прессинга", "Оборона",
        requires=(C.PRESSING_CHAINS,),
...
def pressing_chains(ms: MatchSet) -> dict:
    ...
    return {
        ...
        if "pressing_chain_index" in heads else int(len(heads)),
...
def ppda(ms: MatchSet) -> dict:
    ...
    for mid, g in ev.groupby("match_id"):
        o = g[(g["type"] == "player_possession") & (g["possession_team_id"] != tid) &
              (g["end_type"] == "pass") & (g["x"] > cut)]
        d = g[(g["type"] == "on_ball_engagement") & (g["team_id"] == tid) &
              (g["x"] > cut) & (g["end_type"].notna())]
        if len(d):
            per_match.append({"match": g["match_label"].iloc[0],
                              "ppda": round(len(o) / len(d), 2)})
    return {"value": round(val, 2), "per_match": per_match,
            "clips": episodes(eng_hi.sort_values("t"),
                              lambda r: f"Отбор выше своей трети — {r['player_name']}", 30)}


@metric("regain_zones", "Зоны отбора", "Оборона",
        requires=(C.EVENTS,),
...
def regain_zones(ms: MatchSet) -> dict:
    ...
    tid = ms.subject_team.id
    eng = engagements(ev, tid)
    regains = eng[eng["end_type"].isin(["direct_regain", "indirect_regain"])] \
        if "end_type" in eng else eng.iloc[0:0]
    # Плюс перехваты и подборы, зафиксированные как начало владения.
    own = on_ball(ev, tid)
    starts = own[own["start_type"].isin(
        ["pass_interception", "recovery", "corner_interception",
         "throw_in_interception", "goal_kick_interception"])]
    ...
    return {
        ...
        "clips": episodes(high.sort_values("t"),
                          lambda r: f"Отбор на чужой половине — {r['player_name']}", 30),
    }
