...
@metric("possession_profile", "Владение и фазы атаки", "Атака",
        requires=(C.PHASES,),
...
def possession_profile(ms: MatchSet) -> dict:
    ...
    return {
        ...
        "clips": phase_episodes(
            ...
            lambda r: f"{r['phase']}, {r['duration']:.0f} с, {int(r['n_possessions'])} владений → удар", 30),
    }


@metric("progression", "Продвижение и взлом линий", "Атака",
...
def progression(ms: MatchSet) -> dict:
    ...
    channels = share(last["channel_start"].map(CHANNEL_RU),
    ...
    clips = episodes(
        ...
        lambda r: f"Взлом последней линии — {r['player_name']}, {r.get('phase','')}", 30)
    return {
        "rows": rows,
        "total": int(len(br)),
...
@metric("threat_map", "Карта угрозы", "Атака",
        requires=(C.XTHREAT,),
...
def threat_map(ms: MatchSet) -> dict:
    ...
    ch = [{"key": CHANNEL_RU[c], "value": round(float(ch.get(CHANNEL_RU[c], 0)) / n_matches, 2)}
    ...
    return {
        ...
        "potential_taken": round(
            100 * float(opts[opts["targeted"] == True]["xthreat"].fillna(0).sum())  # noqa: E712
            / max(float(opts["xthreat"].fillna(0).sum()), 1e-9), 1),
        "top_players": [{"name": k, "value": round(float(v) / n_matches, 2)}
                        for k, v in own.groupby("player_name")["xt"].sum()
                        .sort_values(ascending=False).head(8).items()],
        "clips": episodes(own.nlargest(30, "xt"),
                          lambda r: f"{r['player_name']}, xT {float(r['xt']):.3f} — {r.get('phase','')}", 30),
    }


@metric("shots", "Удары", "Атака",
        requires=(C.EVENTS,),
...
def shots(ms: MatchSet) -> dict:
    ...
    return {
        ...
        "by_phase": by_phase,
        "by_start": by_start[:6],
        "top_players": [{"name": k, "n": int(v)} for k, v in
                        sh.groupby("player_name").size().sort_values(ascending=False).head(8).items()],
        "clips": episodes(sh.sort_values("t"),
                          lambda r: f"Удар — {r['player_name']}, {r.get('phase','')}", 40),
        "goal_episodes": episodes(goals_rows.sort_values("t"),
                                  lambda r: f"Гол — атака через {r.get('phase','')}", 20),
    }


@metric("losses", "Потери мяча", "Атака",
        requires=(C.EVENTS,),
...
def losses(ms: MatchSet) -> dict:
    ...
    tid = ms.subject_team.id
    own = on_ball(ev, tid)
    lost = own[own["end_type"].isin(["possession_loss"]) |
               (own["pass_outcome"] == "unsuccessful")] if "pass_outcome" in own else \
    ...
    return {
        ...
        "top_players": [{"name": k, "n": int(v)} for k, v in
                        lost.groupby("player_name").size().sort_values(ascending=False).head(6).items()],
        "clips": episodes(own_third.sort_values("t"),
                          lambda r: f"Потеря в своей трети — {r['player_name']}", 30),
    }


@metric("set_pieces", "Стандарты", "Стандарты",
        requires=(C.EVENTS,),
...
def set_pieces(ms: MatchSet) -> dict:
    ...
    if "player_targeted_y_reception" in corners.columns:
        ...
        targets = [{"x": round(float(r["player_targeted_x_reception"]), 1),
                    "y": round(float(r["player_targeted_y_reception"]), 1),
                    "player": str(r.get("player_targeted_name") or ""),
                    "shot": bool(r.get("lead_to_shot"))}
                   for _, r in c.iterrows()][:60]
    return {
        "rows": rows,
        "total": int(len(sp)),
        "corner_targets": targets,
        "clips": episodes(sp[sp["lead_to_shot"] == True].sort_values("t"),  # noqa: E712
                          lambda r: f"Стандарт с ударом — {SET_PIECE_RU.get(r['game_interruption_before'],'')}", 30),
    }


@metric("time_profile", "Профиль по времени матча", "Ритм",
        requires=(C.XTHREAT,),
...
def time_profile(ms: MatchSet) -> dict:
    ...
    return {
        ...
        "clips_peak": episodes(
            ...
            lambda r: f"{int(r['minute'])+1}′ удар — {r['player_name']}", 30),
        "clips_vuln": episodes(
            opp[(opp["bucket"] == vi) & (opp["end_type"] == "shot")].sort_values("t"),
            lambda r: f"{int(r['minute'])+1}′ удар соперника — {r['player_name']}", 30),
    }
