...
def _get(c: Computed, key: str, *path, default=None):
    v = c.get(key)
    for p in path:
...
def build_insights(c: Computed, bl: Baseline, team_name: str) -> list[Insight]:
    ...
    if forms.get("rows"):
        ...
        out.append(Insight(
            ...
            detail="Прочие схемы: " + ", ".join(
                f"{r['key']} ({r['share']}%)" for r in forms["rows"][1:4]),
        ))
    ...
    if dl.get("mean") is not None:
        ...
        out.append(Insight(
            ...
            comparison=cmp_,
            clips=dl.get("clips", []),
            detail="; ".join(f"{r['phase']} — {r['height']} м" for r in dl.get("by_phase", [])[:4]),
        ))
    ...
    if bs.get("rows"):
        ...
        out.append(Insight(
            ...
            comparison=cmp_w,
            clips=bs.get("clips", []),
            detail="; ".join(f"{r['phase']}: {r['width']}×{r['length']} м" for r in bs["rows"][:4]),
        ))
    ...
    if pp.get("thirds"):
        ...
        out.append(Insight(
            ...
            n=top_third["n"],
            clips=pp.get("clips", []),
            detail="По каналам: " + ", ".join(
                f"{r['key']} {r['share']}%" for r in pp.get("channels", [])),
        ))
    ...
    if pc.get("regain_share") is not None:
        out.append(Insight(
            ...
            evidence=f"{pc.get('n_chains')} цепочек, максимальная — {pc.get('max_length')} действий подряд",
    ...
    if pr.get("possession_share") is not None:
        ...
        out.append(Insight(
            ...
            comparison=cmp_,
            clips=pr.get("clips", []),
            detail="; ".join(f"{r['phase']} {r['share']}% (удар в {r['shot_rate']}%)"
    ...
    if pg.get("rows"):
        ...
        out.append(Insight(
            ...
            text=f"{pg['per_match']} взломов линий за матч; чаще всего вскрывают "
                 f"предпоследнюю линию" if thr else f"{pg['per_match']} взломов линий за матч",
            kind="факт",
            section="Атака",
            ...
            comparison=cmp_,
            clips=pg.get("clips", []),
            detail="; ".join(f"{r['line']}: {r['n']} (через {r['through']}, вокруг {r['around']})"
                             for r in pg["rows"]),
        ))
    ...
    if tm.get("total") is not None:
        ...
        out.append(Insight(
            ...
            clips=tm.get("clips", []),
            detail="По каналам: " + ", ".join(
                f"{r['key']} {r['value']}" for r in tm.get("by_channel", [])),
        ))
    if tm.get("potential_taken") is not None:
        ...
        out.append(Insight(
            ...
            kind="гипотеза",
            section="Атака",
            weight=_weight(-(cmp_["z"]) if cmp_ else 0,
    ...
    if ls.get("own_third_share") is not None and ls["own_third_share"] >= 22:
        out.append(Insight(
            ...
            n=int(ls["total"]),
            clips=ls.get("clips", []),
            detail="Чаще прочих теряют: " + ", ".join(
                f"{r['name']} ({r['n']})" for r in ls.get("top_players", [])[:3]),
        ))
    ...
    if ob.get("per_match"):
        ...
        out.append(Insight(
            ...
            detail="; ".join(f"{r['kind']} {r['n']} (исп. {r['used']}%)"
                             for r in ob.get("rows", [])[:4]),
        ))
    if ob.get("behind_per_match") is not None:
        ...
        out.append(Insight(
            ...
            comparison=cmp_,
            clips=ob.get("clips", []),
            detail="Ломают линию обороны движением в " + str(ob.get("break_line")) +
                   " эпизодах, отодвигают её назад в " + str(ob.get("push_line")) + ".",
        ))
    ...
    if len(rows) == 6:
        ...
        if best["xthreat"] / tot >= 0.22:
            out.append(Insight(
                ...
                detail="; ".join(f"{r['bucket']}: {r['xthreat']} xT / {r['shots']} уд."
                                 for r in rows),
            ))
        ...
        if worst["xthreat_against"] / tot_a >= 0.24:
            out.append(Insight(
                ...
                kind="гипотеза",
                section="Ритм",
                weight=_weight((100 * worst["xthreat_against"] / tot_a - 16.7) / 6,
